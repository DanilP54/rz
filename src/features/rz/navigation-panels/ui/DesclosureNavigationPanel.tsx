    ...
    return (
      <div className="group relative h-[40px]">
        <button
          ...
          onClick={onToggle}
          className={`w-[35px] rounded-none cursor-pointer h-[40px] px-2 ${background_color}`}
        ></button>
        <ul
          id="rz-navigation-panel"
          className={`${
            isExpanded ? "flex" : "hidden"
          } ${background_color} flex-col justify-center gap-3 absolute top-0 left-[40px] right-2 z-50 shadow-lg text-white font-bold text-2xl`}
        >
