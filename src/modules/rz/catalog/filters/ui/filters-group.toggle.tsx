...
import { FilterOption } from "../options/type";

const DEFAULT_OFFSET_ROW = 35;

interface IFiltersToggleGroup<Values> {
  rowChunks: FilterOption<Values | undefined>[][];
  selectedValue?: Values;
  onSelect: (value?: Values) => void;
  offsetRow?: number;
}

export function FiltersToggleGroup<Param extends string>(
  props: IFiltersToggleGroup<Param>
) {
  return (
    <ToggleGroup
      type="single"
      className="flex flex-wrap justify-center items-center w-full rounded-none"
    >
      {props.rowChunks.map((row, index) => (
        <GroupRow
          style={{
            marginLeft: ((props.offsetRow ?? 0) + index) * DEFAULT_OFFSET_ROW,
          }}
          key={index}
        >
          {row.map((option) => {
